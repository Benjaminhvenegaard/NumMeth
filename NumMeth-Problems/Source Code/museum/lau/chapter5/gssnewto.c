...
void gssnewton(int m, int n, float par[], float rv[], float **jjinv,
					...
					float in[], float out[])
{
	...
	void lsqinv(float **, int, float [], int []);
	int i,j,inr,mit,text,it,itmax,inrmax,tim,feval,fevalmax,conv,
			testthf,dampingon,*ci,fail;
	float rho,res1,res2,rn,reltolpar,abstolpar,abstolres,stap,normx,
