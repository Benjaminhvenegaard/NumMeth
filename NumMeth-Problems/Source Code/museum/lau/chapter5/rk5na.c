...
void rk5na(float x[], float xa[], float (*b)(int, float[]),
			...
			int fi, int n, int l, int pos)
{
	...
	void rk5narkstep(float, int, int, float,
					float (*)(int, int, float[]), float [], float [],
					float [], float [], float **);
	int j,i,first,fir,rej,t,ext,extrapolate;
	float fhm,s,s0,cond0,s1,cond1,h,absh,tol,fh,hl,mu,mu1,
...
void rk5narkstep(float h, int d, int n, float mu,
