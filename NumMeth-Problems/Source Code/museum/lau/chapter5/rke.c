...
void rke(float *x, float *xe, int n, float y[],
			void (*der)(int, float, float[]), float data[], int fi,
			void (*out)(int, float, float, float [], float []))
{
	float *allocate_real_vector(int, int);
	void free_real_vector(float *, int);
	int j,last,first,reject,test,ta,tb;
	float xt,h,hmin,ind,hl,ht,absh,fhm,discr,tol,mu,mu1,fh,e1,e2,
			*k0,*k1,*k2,*k3,*k4;
