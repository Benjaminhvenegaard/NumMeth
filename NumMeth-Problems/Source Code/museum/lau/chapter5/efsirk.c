...
void efsirk(float *x, float xe, int m, float y[],
			float *delta, void (*derivative)(int, float[], float *),
			void (*jacobian)(int, float **, float [], float *),
			float **j, int *n, float aeta, float reta, float hmin,
			float hmax, int linear,
			void (*output)(float, float, int, float [],
								float, float **, int))
{
	...
	int k,l,lin,*ri,*ci;
	float step,h,mu0,mu1,mu2,theta0,theta1,nu1,nu2,nu3,yk,fk,c1,c2,
