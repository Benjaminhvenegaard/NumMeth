...
void praxis(int n, float x[], float (*funct)(int, float[]),
				float in[], float out[])
{
	...
	void praxismin(int, int, float *, float *, float *, int, int,
			float [], float **, float *, float *, float *, float,	float,
			float [], float [], int *, int *, float *, float, float,
			float, float, float, float, float, float,
			float (*)(int, float[]));
	int illc,i,j,k,k2,nl,maxf,nf,kl,kt,ktm,emergency;
	float s,sl,dn,dmin,fx,f1,lds,ldt,sf,df,qf1,qd0,qd1,qa,qb,qc,m2,m4,
...
void praxismin(int j, int nits, float *d2, float *x1, float *f1,
			int fk, int n, float x[], float **v, float *qa, float *qb,
			float *qc, float qd0, float qd1, float q0[], float q1[],
			int *nf, int *nl, float *fx, float m2, float m4,
			float dmin, float ldt, float reltol, float abstol,
			float small, float h, float (*funct)(int, float[]))
{
	/* this function is internally used by PRAXIS */

	float praxisflin(float, int, int, float [], float **, float *,
					float *, float *, float, float, float [],
					float [], int *,	float (*)(int, float[]));
	int k,dz,loop;
	float x2,xm,f0,f2,fm,d1,t2,s,sf1,sx1;
...
float praxisflin(float l, int j, int n, float x[], float **v,
			float *qa, float *qb, float *qc, float qd0, float qd1,
			float q0[], float q1[], int *nf,
