...
void eft(float *t, float te, int m0, int m, float u[],
			float (*sigma)(float, int, int), float phi,
			float (*diameter)(float, int, int),
			void (*derivative)(float, int, int, int, float []),
			int *k, float alfa, int norm,
			float (*aeta)(float, int, int),
			float (*reta)(float, int, int),
			float *eta, float *rho, float hmin, float *hstart,
			void (*out)(float, float, int, int, float [],
							int, float, float))
{
	...
	float q,ec0,ec1,ec2,h,hi,h0,h1,h2,betan,t2,sigmal,phil,*c,
