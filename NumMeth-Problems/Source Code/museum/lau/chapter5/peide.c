...
void peide(int n, int m, int nobs, int *nbp, float par[],
		float res[], int bp[], float **jtjinv,
		float in[], float out[],
		int (*deriv)(int,int,float [],float [],float,float []),
		int (*jacdfdy)(int,int,float [],float [],float,float **),
		int (*jacdfdp)(int,int,float [],float [],float,float **),
		void (*callystart)(int,int,float [],float [],float[]),
		void (*data)(int,float [],float [],int[]),
		void (*monitor)(int,int,int,float [],float [],int,int))
{
	...
	int peidefunct(int nrow, int ncol, float par[], float res[],
		int n, int m, int nobs, int *nbp, int first, int *sec,
		int *max, int *nis, float eps1, int weight, int bp[],
		float save[], float ymax[], float y[], float **yp,
		float **fy, float **fp, int cobs[], float tobs[],
		float obs[], float in[], float aux[], int clean,
		int (*deriv)(int,int,float [],float [],float,float []),
		int (*jacdfdy)(int,int,float [],float [],float,float **),
		int (*jacdfdp)(int,int,float [],float [],float,float **),
		void (*callystart)(int,int,float [],float [],float[]),
	...
	void mulcol(int, int, int, int, float **, float **, float);
	...
	float eps1,res1,in3,in4,fac3,fac4,aux[4],*obs,*save,*tobs,
	...
	static float save1[35]={1.0, 1.0, 9.0, 4.0, 0.0, 2.0/3.0, 1.0,
		...
		void (*monitor)(int,int,int,float [],float [],int,int))
{
	...
	void peidereset(int, int, float, float, float, float, float [],
				float [], float *, float *, float *, int *);
	void peideorder(int, int, float, float [], float [],
			float *, float *, float *, float *, float *, int *);
	void peidestep(int, int, int, float, float, float, float,
			float [], float [], float [], float [], int *, float *);
	float peideinterpol(int, int, int, float, float []);
	int l,k,knew,fails,same,kpold,n6,nnpar,j5n,cobsii,*p,evaluate,
			evaluated,decompose,conv,extra,npar,i,j,jj,ii;
	float xold,hold,a0,tolup,tol,toldwn,tolconv,h,ch,chnew,error,
...
void peidereset(int n, int k, float hmin, float hmax, float hold,
		float xold, float y[], float save[], float *ch, float *x,
		float *h, int *decompose)
{
...
void peideorder(int n, int k, float eps, float a[], float save[],
		float *tol, float *tolup, float *toldwn, float *tolconv,
		float *a0, int *decompose)
{
	/* this function is internally used by PEIDEFUNCT of PEIDE */
...
void peidestep(int n, int k, int fails, float tolup, float toldwn,
		float tol, float error, float delta[], float lastdelta[],
		float y[], float ymax[], int *knew, float *chnew)
{
	/* this function is internally used by PEIDEFUNCT of PEIDE */
...
float peideinterpol(int startindex, int jump, int k, float tobsdif,
							float y[])
{
	/* this function is internally used by PEIDEFUNCT of PEIDE */
