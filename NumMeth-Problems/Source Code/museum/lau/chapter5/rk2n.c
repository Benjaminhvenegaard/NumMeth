...
void rk2n(float *x, float a, float b, float y[], float ya[],
			float z[], float za[],
			float (*fxyzj)(int, int, float, float [], float []),
			float e[], float d[], int fi, int n)
{
	float *allocate_real_vector(int, int);
	void free_real_vector(float *, int);
	int j,jj,last,first,reject,test,ta,tb;
	float xl,h,ind,hmin,hl,absh,fhm,discry,discrz,toly,tolz,
