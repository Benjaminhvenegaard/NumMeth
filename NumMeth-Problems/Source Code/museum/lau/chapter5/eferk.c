...
void eferk(float *x, float xe, int m, float y[],
			float *sigma, float phi,
			void (*derivative)(int, float[]),
			float **j,
			void (*jacobian)(int, float **, float [], float *),
			int *k, int l, int aut, float aeta, float reta, float hmin,
			float hmax, int linear,
			void (*output)(float, float, int, float [], float **, int))
{
	...
	float h,b,b0,phi0,cosphi,sinphi,eta,discr,fac,pi,*beta,*betha,
