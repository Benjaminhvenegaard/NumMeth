...
void ark(float *t, float *te, int *m0, int *m, float u[],
			void (*derivative)(int *, int *, float *, float[]),
			float data[],
			void (*out)(int *, int *, float *, float *, float [],
							float []))
{
	...
	float arkmui(int, int, int, float []);
	float arklabda(int, int, int, int, float []);
	static float th1[8] = {1.0, 0.5, 1.0/6.0, 1.0/3.0, 1.0/24.0,
		1.0/12.0, 0.125, 0.25};
	static float ec0,ec1,ec2,tau0,tau1,tau2,taus,t2;
	int p,n,q,start,step1,last,i,j,k,l,n1,m00;
	float thetanm1,tau,betan,qinv,eta,*mu,*lambda,*thetha,*ro,*r,
...
float arkmui(int i, int n, int p, float lambda[])
{
...
float arklabda(int i, int j, int n, int p, float lambda[])
{
	/* this function is internally used by ARK */
