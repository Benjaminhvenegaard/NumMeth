...
int multistep(float *x, float xend, float y[], float hmin, float hmax,
				float ymax[], float eps, int *first, float save[],
				void (*deriv)(float [], int, float, float[]),
				int (*available)(int, float, float [], float **),
				float **jacobian, int stiff, int n,
				void (*out)(float, int, int, float, float []))
{
	...
	void multistepreset(float [], float [], float *, float *,
			float *, float *, int *, float, float,
			float, float, int, int, int);
	void multisteporder(float [], float [], float *, float *,
				float *, float *, float *, int *, float, int, int);
	void multistepstep(int *, float *, float, float, float,
					float [], float,	float [], float [], float [],
					int, int, int, int);
	void multistepjacobian(int, float, float [], float,
			float [], float [], float [], float **,
			void (*)(float [], int, float, float[]),
			int (*)(int, float, float [], float **),
			int *, int *, int *);
	static float adams1[35] = {1.0, 1.0, 144.0, 4.0, 0.0, 0.5, 1.0,
	...
	static float adams2[35] = {1.0, 1.0, 9.0, 4.0, 0.0, 2.0/3.0, 1.0,
	...
	static float xold,hold,a0,tolup,tol,toldwn,tolconv;
	int evaluate,evaluated,decompose,decomposed,conv,i,j,l,k,knew,
			fails,*p;
	float h,ch,chnew,error,dfi,c,a[6],*delta,*lastdelta,*df,**jac,
...
void multistepreset(float y[], float save[], float *x, float *ch,
			float *c, float *h, int *decomposed, float hmin, float hmax,
			float hold, float xold, int m, int k, int n)
{
...
void multisteporder(float a[], float save[], float *tolup,
			float *tol, float *toldwn, float *tolconv, float *a0,
			int *decompose, float eps, int k, int n)
{
	/* this function is internally used by MULTISTEP */
...
void multistepstep(int *knew, float *chnew, float tolup, float tol,
					float toldwn, float delta[], float error,
					float lastdelta[], float y[], float ymax[],
					int fails, int m, int k, int n)
{
	/* this function is internally used by MULTISTEP */

	int i;
	float a1,a2,a3,aa,ss;
...
void multistepjacobian(int n, float x, float y[], float eps,
			float fixy[], float fixdy[], float dy[], float **jacobian,
			void (*deriv)(float [], int, float, float[]),
			int (*available)(int, float, float [], float **),
