...
void rk4na(float x[], float xa[], float (*b)(int, float[]),
			float (*fxj)(int, int, float[]), float e[], float d[],
			int fi, int n, int l, int pos)
{
	...
	void rk4narkstep(float, int, int, int, float,
				float (*)(int, int, float[]), float [], float [],
				float [], float [], float **);
	int j,i,iv,iv0,fir,first,rej,change,t,next,ext,extrapolate;
	float h,cond0,cond1,fhm,absh,tol,fh,max,x0,x1,s,hmin,hl,mu,mu1,
...
void rk4narkstep(float h, int d, int n, int iv, float mu,
				float (*fxj)(int, int, float[]), float x[], float xl[],
				float y[], float discr[], float **k)
{
