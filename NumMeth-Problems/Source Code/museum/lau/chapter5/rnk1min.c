...
float rnk1min(int n, float x[], float g[], float h[],
					float (*funct)(int, float[], float[]),
					float in[], float out[])
{
	...
	void inisymd(int, int, int, float [], float);
	void mulvec(int, int, int, float [], float [], float);
	void dupvec(int, int, int, float [], float []);
	void eigsym1(float [], int, int, float [], float **, float []);
	void linemin(int, float [], float [], float, float *, float [],
					float (*)(int, float[], float[]), float, float *,
					float, float *, int *, int, float []);
	void rnk1upd(float [], int, float [], float);
	void davupd(float [], int, float [], float [], float, float);
	void fleupd(float [], int, float [], float [], float, float);
	int i,it,n2,cntl,cnte,evl,evlmax,ok;
	float f,f0,fmin,mu,dg,dg0,ghg,gs,nrmdelta,alfa,macheps,reltol,
