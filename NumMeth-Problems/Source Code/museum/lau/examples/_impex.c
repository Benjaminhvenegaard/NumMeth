...
void f(float t, float y[], float f1[], int n)
{
	nfe++;
	f1[1]=0.2*(y[2]-y[1]);
...
int available(float t, float y[], float **a, int n)
{
	nje++;
	a[1][1] = -0.2;
	a[1][2] = 0.2;
...
void update(float sw[], float r1[], int n)
{
	int i;
	float s1,s2;
...
void control(float *tp, float t, float h, float hnew, float **y,
				float err[], int n, float tend)
{
	int i;
	float c[6],*x,s,s2,s3,s4;
...
void main ()
{
	void impex(int, float, float, float [],
				void (*)(float, float [], float [], int),
				int (*)(float, float [], float **, int),
				float, float, int, float, float [],
				void (*)(float [], float [], int), int *fail,
	...
	int n,fail,i,it;
	float t,tend,eps,hmax,l,h2,y[4],sw[4],f1[4],f2[4],z[4],x[4],
			n1,n2;
