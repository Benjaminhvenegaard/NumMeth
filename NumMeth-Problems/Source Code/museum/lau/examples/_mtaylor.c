...
float expt,lnt,c0,c1,c2,c3;

void der(float t, int m0, int m, int i, float a[])
{
	if (i == 1) {
		expt=exp(t);
		lnt=log(t);
...
float aeta(float t, int m0, int m)
{
	return 1.0e-5;
}

float reta(float t, int m0, int m)
{
	return 1.0e-4;
}

void op(float t, float te, int m0, int m, float u[],
