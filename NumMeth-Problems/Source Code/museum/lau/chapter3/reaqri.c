...
int reaqri(float **a, int n, float em[], float val[], float **vec)
{
	float *allocate_real_vector(int, int);
	void free_real_vector(float *, int);
	float matvec(int, int, int, float **, float []);
	void rotcol(int, int, int, int, float **, float, float);
	void rotrow(int, int, int, int, float **, float, float);
	int m1,i,i1,m,j,q,max,count;
	float w,shift,kappa,nu,mu,r,tol,s,machtol,elmax,t,delta,det,*tf;
