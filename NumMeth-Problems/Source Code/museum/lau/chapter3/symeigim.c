...
void symeigimp(int n, float **a, float **vec, float val[],
					float lbound[], float ubound[], float aux[])
{
	...
	void orthog(int, int, int, float **);
	void mergesort(float [], int [], int, int);
	void vecperm(int [], int, int, float []);
	void rowperm(int [], int, int, int, float **);
	int qrisym(float **, int, float [], float []);
	float vecvec(int, int, int, float [], float []);
	float matmat(int, int, int, int, float **, float **);
	float tammat(int, int, int, int, float **, float **);
	float infnrmmat(int, int, int, int, int *, float **);
	int k,i,j,i0,i1,i01,iter,maxitp1,stop,n1,i0m1,i1p1,*perm;
	float s,max,tol,mateps,relerra,reltolr,norma,eps2,dl,dr,m1,
