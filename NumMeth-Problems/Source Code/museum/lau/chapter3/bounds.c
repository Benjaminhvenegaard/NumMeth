...
void bounds(int n, float a[], float re[], float im[], float rele,
				float abse, float recentre[], float imcentre[],
				float bound[])
{
	float *allocate_real_vector(int, int);
	void free_real_vector(float *, int);
	void kcluster(int, int, int, float [], float [], float [],
						float [], float [], float [], float []);
	int i,j,k,index1,index2,goon,place,clustin;
	float h,min,recent,imcent,xk,yk,zk,corr,*rc,*c,*rce,*clust,
...
void kcluster(int k, int m, int n, float rc[], float re[],
					float im[], float recentre[], float imcentre[],
					float bound[], float clust[])
{
	/* this function is used internally by BOUNDS */

	int i,stop,l,nonzero;
	float recent,imcent,d,prod,rad,gr,r,*dist,s,h1,h2,temp1,temp2;
