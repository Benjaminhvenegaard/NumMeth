int comeig1(float **a, int n, float em[], float re[], float im[],
				float **vec)
{
	...
	void comscl(float **, int, int, int, float []);
	int comvalqri(float **, int, float [], float [], float []);
	void comveches(float **, int, float, float,
						float [], float [], float []);
	int i,j,k,pj,itt,again,*ind,*ind0;
	float x,y,max,neps,**ab,*d,*u,*v,temp1,temp2;
