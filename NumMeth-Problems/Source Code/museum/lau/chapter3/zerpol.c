...
int zerpol(int n, float a[], float em[], float re[], float im[],
				float d[])
{
	...
	int zerpolfunction(int, float [], float [], float,
							float, float, int *, float *);
	int i,totit,it,fail,start,up,max,giex,itmax,control,ih,m,split;
	float x,y,newf,oldf,maxrad,ae,tol,h1,h2,ln2,f[6],tries[11],
...
int zerpolfunction(int n, float d[], float f[], float x,
						float y, float tol, int *it, float *newf)
{
	/* this function is used internally by ZERPOL */

	int k,m1,m2;
	float p,q,qsqrt,f01,f02,f03,f11,f12,f13,f21,f22,f23,stop;
