...
dot(a,b,n)		returns double value of dot product of two vectors a,b
				of n elements
pv(v,n)			prints vector of n elements

mv(a,x,y,m,n)	y=ax where a is m x n matrix
normv(v,n)		normalize a vector (scale its length to one)
sqnor(v,n)		square of the length of vector v of n elements 
mvt(a,x,y,m,n)	like mv except y=a^x a^ =transpose of a
resid(r,a,x,n)	r=ax-x, a matrix all other vectors.

vs(v,s,n)		scale vector by multiplying each element by s
vset(v,s,n)		set vector v to scalar value s for each element
vcopy(x,y,n)	y=x, vectors 
vv(a,b,c,s,n)	a=b+c*s, s scalar, a,b,c,vectors 
