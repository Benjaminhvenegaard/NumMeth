...
double finv(),tinv(),chiinv(),ncf(),ncc(),incf(),incc(),pow();
...
if(i==2)
	...
	if(x<=0. )
			...
			{
			printf(" noncentral t unsupported-sorry\n");
			}
	}
else if (i==19)
	{
	...
	if(q<=0. )
			...
			{
			printf(" inverse noncentral unsupported sorry\n" );
			}
	}
else if (i==21)
	{
...
for(m=0;m<=itmax;m++)
	...
	delta=d*c;
	h*=delta;
	if(iterkt && abs(delta-1.)<tol)return h*bmult+offset;
	iterkt++;
    };
...
{
double nu1,nu2;
double sqrt(),invnr(),icump(),guess,exp(),p,t,x;
nu1=df1;nu2=0.;
