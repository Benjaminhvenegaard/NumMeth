c This is the main calling program for the DIEHARD battery of tests.
c It is followed by subroutines for the 15 tests and 
c supporting subroutines/functions.
c The file tests.txt is opened and closed often, so be sure it is available.
c Several (Microsoft) compiler-specific routines, which are commented out,
c are for delays or timings and are not essential.
c Note that different compilers have different ways of opening binary stream
c files.  Those here are for DEC Fortran, replacing the original Microsoft.
c Each test subroutine opens and closes the binary test file.       
       character*15 filename,fileout
       character*80 text(36),dum
       integer*2 which(15),ihr,imin,isec,i100
       ksleep=2000
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,244)
       read(5,766) (text(j),j=1,14)
       print 766,(text(j),j=1,14 )
766    format(a80)
       close(5)
1      print*,' Enter filename (<=15 characters):'
C       open(3,file='testout',status='unknown')
       read 845,filename
845    format(a15)
...
C       timetot=isec+60*imin+3600*ihr
C       print 825,ihr,imin,isec
825    format('   Starting time:',i3,':',i2,':',i2)
C       call beepQQ(3000,440)
C       call sleepQQ(3000)
       ...
       write(3,472) fileout
C       call gettim(ihr,imin,isec,i100)
C       print 826,ihr,imin,isec
826    format('   Ending time:',i3,':',i2,':',i2)
C       timetot=isec+60*imin+3600*ihr-timetot
C       jmin=timetot/60.
C       jsec=mod(timetot,60.)
C       print 828,jmin,jsec
828    format('     Duration of tests: ',i3,' min',i3,' sec')
472    format(' Results of DIEHARD battery of tests sent to file ',a15)
C       call beepQQ(4000,440)
       end
...
C If n=2^20, should be 385750 missing 20-letter words, sigma   512
      ...
      real*4 mu,sigma
      open(1,file=filename,form='unformatted',status='old',err=11111
     &,recordtype='stream',carriagecontrol='none')
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,61)
       read(5,766) (text(j),j=1,16)
...
c*********** INITIALIZE*****************
      write(3,362)
      print 362
362   format(' THE OVERLAPPING 20-tuples BITSTREAM  TEST, 20 BITS PER
     &WORD, N words',/,
...
C         *** GET TABLE INDEX FROM LEADING 15 BITS OF J***
             K=ISHFT(J,-5)
C         *** SET BIT L IN W(K) ***
3           W(K)=IOR(W(K),MBIT(L))
C                    ********** COUNT NUMBER OF EMPTY CELLS *******
                      KOUNT=0
                      DO 4 K=0,32767
                      DO 4 L=0,31
4                     IF(IAND(W(K),MBIT(L)).EQ.0) KOUNT=KOUNT+1
C ****END OF MAIN LOOP****
      x=(kount-mu)/sigma
...
C      call sleepQQ(3000)
      return
11111 print*,' File ',filename,' exhausted: no more bits for this test.'
      close(1)
      return
      END
      ...
      n=4000
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,186)
       read(5,766) (text(j),j=1,12)
...
c Put a one-line function here to provide the uni being tested:
     ...
     &,recordtype='stream',carriagecontrol='none')
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,198)
       read(5,766) (text(j),j=1,10)
...
11111 print*,' File ',filename,' exhausted: no more bits for this test.'
     ...
     &,recordtype='stream',carriagecontrol='none')
      ...
      ss=0.
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,151)
       read(5,766) (text(j),j=1,22)
...
c***** one line function to generate a random coordinate in [0,1000).
      uni()=5000.+jtbl()*.2328306e-5
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,173)
       read(5,766) (text(j),j=1,13)
...
C     The Runs-up and Runs-down test.
      ...
      dimension x(1), a(6,6), b(6), ucount(6), dcount(6)
...
C** ONE-LINE FUNCTION TO GENERATE 5-BIT LETTER IN CONVENIENT POSITION
          ikbit()=iand(ishft(jtbl(),-kr),mk)
                  j=1232456789
                  do 879 i=1,1000000
879               j=69069*j
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,77)
       read(5,766) (text(j),j=1,36)
...
C****  GENERATE 2**kpow OVERLAPPING WORDS********
             DO 3 IC=1,2**KPOW
C         *** GET NEW J *****
              j=ishft(iand(j,mkk),lk)+ikbit()
C         *** GET BIT INDEX FROM LAST 5 BITS OF J  ***
             L=IAND(J,31)
C         *** GET TABLE INDEX FROM LEADING 15 BITS OF J***
             Kk=ISHFT(J,-5)
C         *** SET BIT L IN W(Kk) ***
3           W(Kk)=IOR(W(Kk),MBIT(L))
C                    ********** COUNT NUMBER OF EMPTY CELLS *******
                      KOUNT=0
                      DO 4 kk=0,32767
                      DO 4 L=0,31
4                     IF(IAND(W(kk),MBIT(L)).EQ.0) KOUNT=KOUNT+1
C ****END OF MAIN LOOP****
...
c            OBC: overlapping-bit-count from stream of bytes
      integer*4 w,t(0:3124),s(0:624),kbits(-128:127),p(0:4)
      character*15 filename
...
c******that is, this function provides a random keystoke, unequal p's.
        m8()=kbits(jtbl8())
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,113)
       read(5,766) (text(j),j=1,18)
...
c******that is, this function provides a random keystoke, unequal p's.
        m8()=k(iand(ishft(jtbl(),jk-25),255))
        n=10
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,131)
       read(5,766) (text(j),j=1,20)
...
c************ one-line function to get random byte:*************
         i8bit()=iand(ishft(jtbl(),-kr),255)
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,51)
       read(5,766) (text(j),j=1,10)
       print 766,(text(j),j=1,10)
       write(3,766) (text(j),j=1,10)
766    format(a80)
...
11111 print*,' File ',filename,' exhausted: no more bits for this test.'
     ...
     & 1073741824/
       ...
       i=1
33          do 3 ii=i,m
          if(iand(r(ii),msk(j)).eq.msk(j)) then
          x=r(ii)
          r(ii)=r(i)
          r(i)=x
               do 13 k=i+1,m
13           if(iand(r(k),msk(j)).eq.msk(j)) r(k)=ieor(r(k),x)
            rankb=rankb+1
...
11111 print*,' File ',filename,' exhausted: no more bits for this test.'
       ...
       implicit integer(a-z)
       integer r(32),msk(32)
      data (msk(i),i=1,31)/1,2,4,8,16,32,64,128,256,512,1024,2048,4096,
...
212    format(8i10)
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,18)
       read(5,766) (text(j),j=1,15)
...
C A PROGRAM TO DO  THE BIRTHDAY-SPACINGS TEST ON NBITS OF A UNI
      INTEGER B(4096),C(4096),MSPACE(1000),tabl(0:4095)
      real pks(64)
      character*15 filename
      character*80 text(18)
       inbits()=iand(ishft(jb,-kr),mask)
       open(5,file='tests.txt',status='old')
c       read(5,766) (dum,j=1,0)
       read(5,766) (text(j),j=1,18)
       print 766,(text(j),j=1,18)
       write(3,766) (text(j),j=1,18)
766    format(a80)
...
c  32  format(' UNI = ',f8.5,' (Probability of a better fit.)')
     ...
     &,recordtype='stream',carriagecontrol='none')
       m=100
       open(5,file='tests.txt',status='old')
       read(5,766) (dum,j=1,208)
       read(5,766) (text(j),j=1,11)
